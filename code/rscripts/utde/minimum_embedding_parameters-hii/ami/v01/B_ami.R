...
dataset_path <- '/dataset/hii'

### Outcomes Data Path
outcomes_data_path <- paste(github_repo_path,'/data-outputs', feature_path, '/v', version, sep="")
### Data Path
data_path <- paste(github_repo_path,'/data-outputs', dataset_path, '/v', data_version, sep="")
...
################################################################################
# (2) Reading data
file_ext <- paste('xdata_v', data_version, '.dt',sep='')
...
xdatam <- xdatam[.(c('HNnb'))]

} else if (movement_variable == 'HFnb' ) {
setkey(xdatam, Activity)
xdatam <- xdatam[.(c('HFnb'))]

} else if (movement_variable == 'HNwb' ) {
setkey(xdatam, Activity)
xdatam <- xdatam[.(c('HNwb'))]

} else if (movement_variable == 'HFwb' ) {
setkey(xdatam, Activity)
xdatam <- xdatam[.(c('HFwb'))]

} else if (movement_variable == 'VNnb') {
setkey(xdatam, Activity)
xdatam <- xdatam[.(c('VNnb'))]

} else if (movement_variable == 'VFnb') {
setkey(xdatam, Activity)
xdatam <- xdatam[.(c('VFnb'))]

} else if (movement_variable == 'VNwb') {
setkey(xdatam, Activity)
xdatam <- xdatam[.(c('VNwb'))]

} else if (movement_variable == 'VFwb') {
setkey(xdatam, Activity)
xdatam <- xdatam[.(c('VFwb'))]
...
xdatam <- xdatam[.(c('HS01', 'HS02'))] # HumanSensor01, HumanSensor02
...
axis <- names(xdatap)[5: (  length(xdatap))  ]
xpa <- xdatap
...
hs02xpak <- xpak[Sensor=='HS02', .SDcols=cols  ]
...
for (axis_k in c(1:length(axis)  )){ #for (axis_k in c(1:length(axis))){
	...
	########################
	message('     #HS02')
	inputtimeseries <- hs02xpak[,  get(axis[axis_k]) ]
...
#tagfile <- paste('-',movementtag,'-', windowltag, '.dt', sep='')
